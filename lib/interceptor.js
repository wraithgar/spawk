...
function compareOptions (defined, called) {
  const flat = ['cwd', 'argv0', 'detached', 'uid', 'gid', 'serialization',
    'shell', 'windowsVerbatimArguments', 'windowsHide', 'signal']
  for (const f of flat) {
    if (Object.prototype.hasOwnProperty.call(defined, f) && defined[f] !== called[f]) {
      return false
    }
  }
  if (Object.prototype.hasOwnProperty.call(defined, 'stdio') && (JSON.stringify(defined.stdio) !== JSON.stringify(called.stdio))) {
    return false
  }
...
class Interceptor {
  ...
  match (command, args = [], options = {}) {
    ...
    if (this.options && !compareOptions(this.options, options)) {
      return false
    }
